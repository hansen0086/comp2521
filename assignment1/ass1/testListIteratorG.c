...
int main(int argc, char *argv[])
{
  ...
  it = IteratorGNew(stringCompare, stringNew, stringFree);
  done = 0;
  while (!done) {
    com = getCommand();
    switch (com) {
  ...
  if(it->first!=NULL)
  {
  printf("The first is %s\n",(char*)it->first->element);
  }else{
   printf("The first is NULL\n");
  }
  if(it->last!=NULL)
  {
   printf("The last is %s\n",(char*)it->last->element);
  }
  else{
  ...
  if(it->cursprev!=NULL)
  {
  printf("The left cursor is %s\n",(char*)it->cursprev->element);
  }
  else
  {
   printf("The left cursor is null\n");
  }
  if(it->cursnext!=NULL)
  {
  printf("The right cursor is %s\n",(char*)it->cursnext->element);
  
   }
   else
   {
      printf("The right cursor is null\n");
   }
}  
