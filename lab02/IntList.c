...
void IntListInsertInOrder(IntList L, int v)
{
    ...
    else
   {
      ...
      struct IntListNode *curr = prev->next;
            while(curr!=NULL)
           {
                if(v <= curr->data)// in the middle or somewhere near..
                {
                     prev->next = new_node;
                     new_node->next = curr;
                     break;
   ...
   return;
	...
	/*if(L->size == 0)//nothing in the list
	{
	        IntListInsert(L, v);
	        return;
	}
	...
	{
	       if(v <= L->first->data)//at least one thing in the list, and should insert the data into the frist place
	       {
	                struct IntListNode *new_first;
	                new_first = newIntListNode(v);
	                new_first -> next = L -> first;
	                L->first = new_first;
	                L->size++;
	                return;
	       }
	       else//the new node should be inserted in the somewhere middle or at the end of the list;
	       {
	                struct IntListNode *new_node= newIntListNode(v);
	                struct IntListNode *prev = L->first;
	                struct IntListNode *curr = prev->next;
